                ...
                <div className={styles.topHeadlines}>
                  {
                    featuredNewsLoaded ? JSON.parse(featuredNews).map((i)=>{
                      return <Link key={i[0].title} style={{textDecoration:'none'}} to={'/news/'+i[0].id} onClick={()=>{window.location.reload()}}><Headline img={i[0].img} title={i[0].title.split(" ").splice(0,11).join(" ")+"..."} text={i[0].smallBody.split(" ").splice(0,11).join(" ")+'...'} /></Link>
